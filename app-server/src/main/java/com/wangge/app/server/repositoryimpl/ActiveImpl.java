...
public class ActiveImpl {
  ...
  * @throws
  ...
  public  Double examTwiceShop(String memberId){
    String sql = "select count(*) from BIZ_ACTIVE c where c.n > =2 and c.member_id = '"+memberId+"'";
    Query query =  em.createNativeQuery(sql);
    Object s =   query.getSingleResult(); 
    return  Double.parseDouble(s.toString());
  }
 ...
 */
  public Double examOneceShop(String memberId) {
    String sql = "select count(*) from BIZ_ACTIVE c where c.n> =1 and c.member_id in  '"+memberId+"'";
    Query query =  em.createNativeQuery(sql);
    Object s =  query.getSingleResult(); 
    return  Double.parseDouble(s.toString());
  }
