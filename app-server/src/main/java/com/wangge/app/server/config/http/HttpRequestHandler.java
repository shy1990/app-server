...
//        System.out.println(headers);
  ...
  private void printInfoLog(String url, Object requestParams, Object resultObject) {
    ...
    if (StringUtils.isNotBlank(url)) {
	    logger.info(MessageFormat.format("{0}{1}", "++++++++++++++Request Url:", url));
    }

    if (requestParams != null) {
	    logger.info(MessageFormat.format("{0}{1}", "++++++++++++++Request Params:", JSONObject
       .toJSONString(requestParams, true)));
    }

    if (resultObject != null) {
	    logger.info(MessageFormat.format("{0}{1}", "++++++++++++++Request Result:", JSONObject
