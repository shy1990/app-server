...
public class OrderSignforImpl {
	 ...
	 * 
	 * @Title: getOrderList @Description: TODO(这里用一句话描述这个方法的作用) @param @param
	 *         userPhone @param @param type @param @param pageNo @param @param
	 *         pageSize @param @return 设定文件 @return QueryResult
	 *         <OrderSignfor> 返回类型 @throws
	 */
	public QueryResult<OrderSignfor> getOrderList(String userPhone, String type, int pageNo, int pageSize) {
		String sql = "";
		if ("0".equals(type)) {
			sql = "select  os.order_no,os.shop_name,os.creat_time,os.order_status,os.phone_count,os.order_price  from BIZ_ORDER_SIGNFOR os  where os.user_phone = '"
					+ userPhone + "' order by os.signid";
		} else {
			sql = "select  os.order_no,os.shop_name,os.creat_time,os.order_status,os.phone_count,os.order_price  from BIZ_ORDER_SIGNFOR os  where os.user_phone = '"
					+ userPhone + "' and os.order_status in (2,3,4) order by os.signid desc";
		}
		...
		List<OrderSignfor> olist = getSignOrderList(query.getResultList(), start, end);
		QueryResult<OrderSignfor> qr = new QueryResult();
		qr.setContent(olist);
		qr.setTotalPages(total, pageSize);
		// Page<OrderSignfor> page = new PageImpl<OrderSignfor>(olist, new
		// PageRequest(pageNo, pageSize), olist.size());
		return qr;
	}
	 ...
	 * @throws ParseException
	 * 
	 * @Title: getOrderSignforList @Description:
	 *         TODO(这里用一句话描述这个方法的作用) @param @param userPhone @param @param
	 *         pageNo @param @param pageSize @param @return 设定文件 @return
	 *         QueryResult<OrderSignfor> 返回类型 @throws
	 */
	public QueryResult<OrderSignfor> getOrderSignforList(String userPhone, int pageNo, int pageSize) {
		String sql = "select t.* from (select os.fastmail_no,os.yewu_signfor_time, os.creat_time,count(os.signid) as orderCount from biz_order_signfor os  where os.user_phone = '"
				+ userPhone
				+ "'  group by os.fastmail_no, os.yewu_signfor_time, os.creat_time  having count(os.signid) > 1 ) t order by t.creat_time";
		Query query = em.createNativeQuery(sql);
	...
	private List<OrderSignfor> getSignOrderList(List olist, int start, int end) {
		...
		Object[] o = null;
		OrderSignfor os = null;
		for (int i = start; i < end; i++) {
			os = new OrderSignfor();
			o = (Object[]) olist.get(i);
	...
	private List<OrderSignfor> createOrderList(List olist) {
		...
		/*
		 * for(int i = 0;i<olist.size();i++){ OrderSignfor o = new
		 * OrderSignfor();
		 * System.out.println("============"+String.valueOf(olist.get(0)));
		 * o.setOrderNo(String.valueOf(olist.get(0)));
		 * o.setShopName(String.valueOf(olist.get(1))); //o.setCreateTime(new
		 * Date(Long.parseLong(String.valueOf(olist.get(2))) * 1000)); //
